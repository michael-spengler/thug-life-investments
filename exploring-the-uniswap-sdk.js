...
const tokenAddress = '0x0d8775f648430679a709e98d2b0cb6250d2887ef' // BAT
  ...
  console.log(token)
  const etherOnMainnet = WETH[chainId]
  const pair = await Fetcher.fetchPairData(token, etherOnMainnet)
  console.log(pair)
  const route = new Route([pair], etherOnMainnet)
  const trade = new Trade(route, new TokenAmount(etherOnMainnet, '100000000000000'), TradeType.EXACT_INPUT)
  ...
  const path = [etherOnMainnet.address, token.address]
  const to = ''
  ...
  const uniswap = new ethers.Contract(
    ...
    const transaction = await uniswap.swapExactETHForTokens(amountOutIn, path, to, deadline, {value, gasPrice: 20e9})
